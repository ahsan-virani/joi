...
module.exports = internals.Any = class {
  ...
  createError(type, context, state, options) {
    ...
    return Errors.create(type, context, state, options, this._flags, this._customErrorMessage);
  }
  ...
  applyFunctionToChildren(children, fn, args, root) {
    ...
    if (children.length !== 1 || children[0] !== '') {
      ...
      const extraChildren = (children[0] === '' ? children.slice(1) : children)
        .map((child) => {
  ...
  customMessage(errMessage) {
    Hoek.assert(errMessage && typeof errMessage === 'string', 'Error message must be a non-empty string');

    const obj = this.clone();
    obj._customErrorMessage = errMessage;
    return obj;
  }
  ...
  _validate(value, state, options, reference) {
    ...
    if (this._flags.empty && !this._flags.empty._validate(this._prepareEmptyValue(value), null, internals.defaults)
      .errors) {
      value = undefined;
    }
